  ...
  template: `
    ...
    >
      ...
      <div class="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10 my-auto">
        ...
        <div class="lg:col-span-7 flex justify-center lg:justify-start order-2 lg:order-1 relative select-none">
          <!-- Background Radial Glow -->
          <div class="absolute inset-0 bg-[radial-gradient(circle,rgba(224,122,95,0.08)_0%,transparent_75%)] pointer-events-none"></div>

          <!-- Product: Junior Active Sneakers -->
          <div *ngIf="sneakerProduct" class="editorial-float flex flex-col space-y-4 relative z-10 max-w-md w-full">
            <div class="relative w-full aspect-[4/5] rounded-[2rem] overflow-hidden border border-[#2A2522]/5 shadow-2xl shadow-black/10 group pointer-events-auto">
              ...
              <div class="absolute inset-0 bg-[#110F0E]/40 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-center items-center p-6 text-center z-20">
                <span class="text-[9px] font-mono tracking-widest text-[#E07A5F] uppercase font-bold mb-2">Ref. 04 / KIDS</span>
                <h4 class="text-sm font-light text-white uppercase tracking-wide mb-1">{{ sneakerProduct.title }}</h4>
                <span class="text-xs font-mono text-white/90 font-bold mb-4">{{ sneakerProduct.price | currency:'EGP ' }}</span>
                <a [routerLink]="['/products', sneakerProduct.id]" class="px-5 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40 text-white text-[9px] font-bold uppercase tracking-widest rounded-xl transition-all">
                  [ Quick Shop ]
                </a>
              </div>
            </div>
        ...
        <div class="lg:col-span-5 flex flex-col items-start lg:items-end text-left lg:text-right order-1 lg:order-2 space-y-6">
          ...
          <div class="w-16 h-[1.5px] bg-[#E07A5F] my-2 lg:ml-auto"></div>
          <p class="font-sans text-xs md:text-sm text-[#6B5E57] font-light leading-relaxed max-w-md select-none">
            Lightweight, high-traction soles and soft knit uppers. Ergonomic support meets contemporary design for little feet in motion.
          </p>
          <div class="pt-4 pointer-events-auto">
            ...
            >
              <span class="relative z-10">Explore Kids</span>
